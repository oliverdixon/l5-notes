...
% tcolorboxes, for which a fix was proposed by the author of the latter:
...
\ifdraft
    \fancypagestyle{plain}{
        \fancyhf{}
        \fancyhead[C]{\textbf{DRAFT/UNFINISHED\quad\today\quad
            All rights reserved}}
        \fancyfoot[C]{\thepage}
    }
\fi
...
\fancypagestyle{mainbody}{
    ...
    \ifdraft\fancyfoot[C]{\bfseries DRAFT/UNFINISHED\\\today\quad All rights
        reserved}\fi
}
\makeatother
...
\begin{definition}{Global continuity (topological perspective)}
    ...
    and only if $ f^{-1}(V) \subseteq X $ is also open. We can eventually
    restate this definition in terms of a closed set $ F \subseteq Y $:
    \begin{align}
    ...
    \begin{tikzpicture}
        ...
        \end{scope}
        \node[below of=leftspace, yshift=-1cm] {$ (X, d) $};
        \node[below of=rightspace, yshift=-1cm] {$ (Y, e) $};
