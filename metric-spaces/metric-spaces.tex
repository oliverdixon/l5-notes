...
\newcommand\lecture[4]{
    \section{#1}\hfill
    \raisebox{3pt}{
        \ifstrempty{#2}{\textit{No recording}}{
            \textit{#4}\hspace*{8pt}
            \href{https://york.cloud.panopto.eu/Panopto/Pages/#2}
                {\fbox{\centering\includegraphics[width=13pt]{../panopto}}}
        }%
    }%
    \par #3
...
\begin{example}{Illustration of Interior, Boundary, and Exterior Points}
    ...
    \begin{minipage}{.3\linewidth}
        ...
        \begin{tikzpicture}[scale=0.95]
            ...
            \pic[scale=0.5, every path/.style={fill=ballfill, dashed}] at (0, 0)
                {squarespace};
            \filldraw (0, 0) circle (1pt) node[below, yshift=-5pt] {$ x $};
        \end{tikzpicture}
    ...
    \begin{minipage}{.3\linewidth}
        ...
        \begin{tikzpicture}[scale=0.95]
            ...
            \pic[scale=0.5, every path/.style={fill=ballfill, dashed}] at (0, 0)
                {squarespace};
            \filldraw (.75, 0) circle (1pt) node[right, xshift=5pt] {$ y $};
        \end{tikzpicture}
    ...
    \begin{minipage}{.3\linewidth}
        ...
        \begin{tikzpicture}[scale=0.95]
            ...
            \pic[scale=0.5, every path/.style={fill=ballfill, dashed}] at (0, 0)
                {squarespace};
            \filldraw (1.3, 0) circle (1pt) node[below, yshift=-5pt] {$ z $};
        \end{tikzpicture}
    ...
    \begin{minipage}{.5\linewidth}
        ...
        \begin{tikzpicture}
            ...
            \draw (0, 0) circle[radius=1.3] node {$ A^o $} node[above left=1cm]
                {$ A^e $};
            \foreach \i in {0, 30, ..., 360} \filldraw (\i:1.15cm) circle (1pt);
        \end{tikzpicture}
...
\begin{theorem}{Equivalence between openness and the existence of open balls}
    ...
    \begin{minipage}{.4\linewidth}
        ...
        \begin{tikzpicture}
            ...
            \foreach \i in {1, 2, 3}{
                \filldraw (x\i point) circle (1pt);
                \draw[dashed] (x\i point) circle (.4cm*\i+.05cm) node[below]
                    {$ x_\i $};
                \draw (x\i point) -- ++(45*\i:.4cm*\i+.05cm);
            }
        \end{tikzpicture}
    \end{minipage}
\end{theorem}
    ...
    \begin{proof}
        ...
        \begin{minipage}{.5\linewidth}
            ...
            \begin{tikzpicture}
                ...
                \filldraw (xpoint) circle (1pt) node[left, xshift=-3pt]{$ x $};
                \filldraw (ypoint) circle (1pt) node[right, xshift=3pt]{$ y $};
                \draw (xpoint) -- (ypoint);
                \draw[dashed] (ypoint) circle (1cm);
                \draw (ypoint) -- ++(120:1cm) node[left, pos=.5, yshift=-3pt]
                    {$ \frac{\epsilon^\prime}{2} $};
                \node at (-.75, -1.3) {$ \left[\,\Delta = d(x, y)\,\right] $};
            \end{tikzpicture}
        \end{minipage}
...
\begin{example}{Non-finite open sets are not closed under intersection}
    ...
    \begin{tikzpicture}
        ...
        \draw[<->] (linestart) -- (lineend) node[right, xshift=7pt]
            {$ \mathbb{R} $};
        \draw (0, .2) -- (0, -.2) node[above, yshift=15pt] {$ 0 $};
        \foreach \i in {1, ..., 4}{
            \draw (-5+\i, .2) arc[start angle=90, end angle=270, radius=.2]
                node[above, xshift=-5pt, yshift=13pt] {$ -\frac{1}{\i} $};
            \draw (5-\i, .2) arc[start angle=90, end angle=-90, radius=.2]
                node[above, xshift=3pt, yshift=13pt] {$ \frac{1}{\i} $};
            \draw[<->] (-5+\i, -\i*.5-.5) -- (5-\i, -\i*.5-.5);
            \draw[dotted] (-5+\i, -\i*.5-.5) -- (-5+\i, 0);
            \draw[dotted] (5-\i, -\i*.5-.5) -- (5-\i, 0);
            \node at (5, -\i*.5-.5) {$ I_\i $};
        }
    \end{tikzpicture}
...
\begin{example}{\texorpdfstring{$ \mathbb{R}^2 $}{The plane} is topologically
    ...
    of $ \Omega $ by the $ d_2 $ metric. Thus, $ \Omega \in T_{d_2} $.
    ...
    lie entirely within the Euclidean $ d_2 $ unit circle. Intuitively, we can
    see that the metrics $ d_1 $ and $ d_2 $ induce the same topologies; an
    analogous argument applies for establishing the equivalences to $ d_\infty
    $.
    ...
    \begin{minipage}{.45\linewidth}
        ...
        \begin{tikzpicture}
            ...
            \filldraw[red] (x0point) circle (1pt) node[left, red, xshift=-3pt,
                yshift=-1pt] {$ x_0 $};
        \end{tikzpicture}
    ...
    \begin{minipage}{.45\linewidth}
        ...
        \begin{tikzpicture}
            ...
            \foreach \i in {0, 1, 2} {
                ...
                \filldraw[red] (x\i point) circle (1pt) node[below] {$ x_\i $};
            }
        \end{tikzpicture}
...
\begin{definition}{Closure}{closure}
    ...
    \begin{tikzpicture}
        ...
        \draw[fill=ballfill, dotted] (leftball) circle[radius=1] node {$ A $}
            node [above left=1cm] {$ X $};
        \foreach \i in {0, 30, ..., 360} \filldraw (leftball)++(\i:1cm)
            circle (1pt);
        \pic at (rightball) {squarespace};
        \draw[fill=ballfill] (rightball) circle[radius=1] node
            {$ \overline{A} $} node [above left=1cm] {$ X $};
        \foreach \i in {0, 30, ..., 360} \filldraw (rightball)++(\i:1cm) circle
            (1pt);
        \draw[->, thick] (leftbound) -- (rightbound) node [above, pos=.5,
            yshift=7pt] {$ A \mapsto A \cup \partial A = \overline{A} $} node
            [below, pos=.5, yshift=-7pt] {Closure Operation on $ A $};
    \end{tikzpicture}
